...
def objective_function_wrapper(rn_params_array, options_data, p_params):
    ...
    #return objective_function_parallel(rn_params, options_data, p_params)


def objective_function_parallel(rn_params: Dict, options_data, p_params: Dict):
    #IV minimisation
    def process_row(row):
        K = row['Strike']
        ...
        observed_IV = row['IV']
        model_IV = implied_volatility(model_price, Y_0, K, s/365, r, flag)
         
        return (model_IV - observed_IV) ** 2

    squared_diff_sum = 0
    with ThreadPoolExecutor() as executor:
        squared_diff_results = executor.map(process_row, options_data)
        squared_diff_sum = sum(squared_diff_results)
...
def objective_function_numba(rn_params: Dict, options_data, p_params: Dict):
    ...
    for i in prange(options_data.shape[0]):
        ...
        observed_price = row['European_price']
        model_price = call_option_price(rn_params, K, Y_0, V_0, 0, s, v_st, p_params)
        squared_diff_sum += (model_price - observed_price) ** 2
    print(squared_diff_sum)
    return squared_diff_sum
...
def call_option_price(rn_params, K, Y_0, V_0, t, s, v_st, p_params):
    integral = simpsons_rule_integration(integrand, 10e-5, 60, 1000, K, Y_0, V_0, t, s, v_st, p_params, rn_params)
